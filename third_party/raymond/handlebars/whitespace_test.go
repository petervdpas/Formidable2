...
var whitespaceControlTests = []Test{
	...
	// ── regression: adjacent standalone block tags ────────────────
	// In handlebars.js this template renders contiguously. Raymond's
	// original whitespace visitor checked node.Value (post-strip)
	// instead of node.Original when deciding whether the *next* block
	// was preceded by whitespace. After the closing `{{/with}}` strips
	// the leading "\n" off the inter-block content, the following
	// `{{#each}}` would conclude "no preceding whitespace" and skip
	// its standalone strip — leaving stray indent + a blank line
	// between the with-body and the each-body, AND a blank line
	// between consecutive each iterations because the each's program
	// body kept its leading "\n".
	{
		"adjacent standalone block tags strip inter-block whitespace",
		"  {{#with x}}\n|hdr|\n  {{/with}}\n  {{#each rows}}\n|{{this}}|\n  {{/each}}",
		map[string]any{
			"x":    map[string]any{"_": 1},
			"rows": []any{"a", "b"},
		},
		nil, nil, nil,
		"|hdr|\n|a|\n|b|\n",
