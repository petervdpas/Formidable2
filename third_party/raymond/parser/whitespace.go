...
func isPrevWhitespaceProgram(body []ast.Node, i int, isRoot bool) bool {
	...
	if node, ok := prev.(*ast.ContentStatement); ok {
		...
		// Use the unmutated Original so this check stays correct after a
		// preceding standalone block has already stripped trailing whitespace
		// off node.Value. Mirrors handlebars.js's whitespace-control.js,
		// which tests `prev.original`, not the live (post-strip) value.
		return r.MatchString(node.Original)
	}
...
func isNextWhitespaceProgram(body []ast.Node, i int, isRoot bool) bool {
	...
	if node, ok := next.(*ast.ContentStatement); ok {
		...
		// See isPrevWhitespaceProgram — same Original-vs-Value rationale.
		return r.MatchString(node.Original)
	}
