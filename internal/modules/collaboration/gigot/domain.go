...
// ── HTTP-bound surface (stubbed during scaffold) ────────────────────

// Ping issues GET /api/health. Stubbed — wiring lands in the next pass.
...
// Context issues GET /api/repos/{repo}/context. Stubbed.
...
// Formidable issues GET /api/repos/{repo}/formidable. Stubbed.
...
// Head issues GET /api/repos/{repo}/head. Stubbed.
...
// Tree issues GET /api/repos/{repo}/tree. Stubbed.
...
// GetFile issues GET /api/repos/{repo}/files/{path}. Stubbed.
...
// Log issues GET /api/repos/{repo}/log?limit=N. Stubbed.
...
// Destinations issues GET /api/repos/{repo}/destinations. Stubbed.
...
func (m *Manager) DestinationSync(conn Connection, destinationID string) (*Destination, error) {
	...
	if destinationID == "" {
		return nil, fmt.Errorf("gigot: missing destination id")
	}
	return nil, ErrNotImplemented
}

// Commit issues POST /api/repos/{repo}/commits. Stubbed.
...
// PushLocal walks the context folder, diffs against the track-record,
// and commits changed files to the server. Stubbed.
...
// Sync runs PushLocal then PullLocal. Stubbed.
