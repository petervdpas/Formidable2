...
var builtinHelpers = []HelperDescriptor{
	...
	// ── field accessors ──────────────────────────────────────────
	{Name: "board", Signature: `{{board [scope=false] [columns="a,b"]}}`, Description: "Render a Project Mode plan board: a mermaid Gantt framed to the project window, then a GFM table of its events. `scope=false` drops the frame and auto-fits to the events. `columns` picks which loop fields become table columns (comma list of keys, in that order; `columns=false` shows only the four event axes).", Example: `{{board}}`, Category: HelperCategoryField},
	{Name: "boardSlices", Signature: `{{#boardSlices [count=N] [size=N] [trim=true] [columns="a,b"]}}…{{/boardSlices}}`, Description: "Block helper: paginate the plan board for print, one iteration per page exposing `gantt`, `table`, `index`, `total`, `isFirst`, `isLast`, `from`, `to`. `count` = pages, `size` = columns per page; default tiles the whole project window, `trim=true` hugs only the weeks with events. `columns` picks which loop fields become table columns, same as `{{board}}`.", Example: "{{#boardSlices count=2}}\n{{{gantt}}}\n\n{{{table}}}\n{{#unless isLast}}<div class=\"page-break\"></div>{{/unless}}\n{{/boardSlices}}", Category: HelperCategoryField},
	{Name: "boardMeta", Signature: `{{boardMeta "prop" [unit]}}`, Description: "Read one scalar off the plan board: `name`, `from`, `to`, `timeblock`, `duration` (days, or `weeks`/`months`), `ticks`, `events`, `resources`. Empty for an unknown prop.", Example: `Duration: {{boardMeta "duration" "weeks"}} weeks`, Category: HelperCategoryField},
	{Name: "slide", Signature: `{{slide}}`, Description: "Render this record's slide canvas (presentation mode) to positioned HTML, without naming the field key. The presentation-mode companion to `{{board}}`. Empty when the template has no slide field.", Example: `{{slide}}`, Category: HelperCategoryField},
