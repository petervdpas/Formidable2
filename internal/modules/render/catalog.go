...
var builtinHelpers = []HelperDescriptor{
	...
	// ── field accessors ──────────────────────────────────────────
	{Name: "board", Signature: `{{board}}`, Description: "Render the plan-board record (Project Mode) as a mermaid Gantt chart followed by a GFM table of its events. Sections are the project's resources; each event is a task. The table carries the four event axes (resource, kind, start, end) plus one column per author-added events-loop field. Empty when there are no events.", Example: `{{board}}`, Category: HelperCategoryField},
