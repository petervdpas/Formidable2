...
var builtinHelpers = []HelperDescriptor{
	...
	// ── field accessors ──────────────────────────────────────────
	{Name: "board", Signature: `{{board [scope=false]}}`, Description: "Render a Project Mode plan board: a mermaid Gantt framed to the project window, then a GFM table of its events. `scope=false` drops the frame and auto-fits to the events.", Example: `{{board}}`, Category: HelperCategoryField},
	{Name: "boardSlices", Signature: `{{#boardSlices [count=N] [size=N] [trim=true]}}…{{/boardSlices}}`, Description: "Block helper: paginate the plan board for print, one iteration per page exposing `gantt`, `table`, `index`, `total`, `isFirst`, `isLast`, `from`, `to`. `count` = pages, `size` = columns per page; default tiles the whole project window, `trim=true` hugs only the weeks with events.", Example: "{{#boardSlices count=2}}\n{{{gantt}}}\n\n{{{table}}}\n{{#unless isLast}}<div class=\"page-break\"></div>{{/unless}}\n{{/boardSlices}}", Category: HelperCategoryField},
