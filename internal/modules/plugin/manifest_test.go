...
func TestNormalizeManifest_LogAsToastForcesHideLog(t *testing.T) {
	// log_as_toast and the inline log panel are mutually exclusive
	// (toasts ARE the log surface when this flag is on). normalize
	// snaps hide_log=true so a manifest can't carry the contradictory
	// "show inline AND surface as toasts" combo.
	m := Manifest{
		ManifestVersion: 1,
		ID:              "p",
		Name:            "P",
		Version:         "0.1.0",
		Commands: []Command{
			{ID: "a", Label: "A", LogAsToast: true, HideLog: false},
			{ID: "b", Label: "B", LogAsToast: false, HideLog: false},
			{ID: "c", Label: "C", LogAsToast: true, HideLog: true},
		},
	}
	normalizeManifest(&m)
	if !m.Commands[0].HideLog {
		t.Fatalf("a: log_as_toast=true should force hide_log=true; got %+v", m.Commands[0])
	}
	if m.Commands[1].HideLog {
		t.Fatalf("b: untouched when log_as_toast=false; got %+v", m.Commands[1])
	}
	if !m.Commands[2].HideLog || !m.Commands[2].LogAsToast {
		t.Fatalf("c: already-consistent flags should be preserved; got %+v", m.Commands[2])
	}
}

func TestLoadManifest_NormalizesOnRead(t *testing.T) {
	// A hand-edited manifest with log_as_toast=true but hide_log=false
	// must come out of LoadManifest already snapped, so the runtime
	// never sees the inconsistent pair.
	...
	if !got.Commands[0].HideLog {
		t.Fatalf("expected hide_log=true after normalize, got %+v", got.Commands[0])
	}
}
