Feature: Template management
  ...
  Scenario: A plan board validates cleanly
  ...
  Scenario: Project Mode needs a project field
    Given a template in project mode with fields:
      | key | type |
      | t   | text |
    Then validation reports a "project-mode-needs-project" error

  Scenario: Event field needs Project Mode
    Given a template with fields:
      | key    | type      |
      | events | loopstart |
      | event  | event     |
      | events | loopstop  |
    Then validation reports a "event-needs-project-mode" error

  Scenario: Event field needs an events loop
    Given a template in project mode with fields:
      | key     | type    |
      | project | project |
      | event   | event   |
    Then validation reports a "event-needs-events-loop" error
