...
// ─────────────────────────────────────────────────────────────────────
// Generator — port of utils/templateGenerator.js
//
// Four shapes:
//   - report:      frontmatter + per-field heading + value + debug log section
//   - minimal:     per-field heading + value, no frontmatter, no logs
//   - table:       single key/value Markdown table
//   - frontmatter: YAML data block only
//
// All shapes must produce text that compiles under render.Manager
// (raymond + goldmark + chroma) so the generated output can be
...
func TestGenerate_EmptyFieldsAllShapes(t *testing.T) {
	for _, shape := range []Shape{ShapeReport, ShapeMinimal, ShapeTable, ShapeFrontmatter} {
		got := GenerateMarkdownTemplate(shape, nil)
		if got != "" {
			t.Errorf("shape %q empty fields: want \"\", got %q", shape, got)
		}
	}
}

func TestGenerate_UnknownShapeFallsBackToReport(t *testing.T) {
	got := GenerateMarkdownTemplate("does-not-exist", []Field{{Key: "x", Type: "text"}})
	if !strings.Contains(got, "title: Auto-generated Report") {
		t.Fatalf("unknown shape should fall back to report; got:\n%s", got)
	}
}

// ─── Report shape (the original templateGenerator.js behavior) ────────

func TestGenerate_ReportHasFrontmatterAndDebugLog(t *testing.T) {
	got := GenerateMarkdownTemplate(ShapeReport, sampleFields())
...
func TestGenerate_ReportBooleanUsesIfElse(t *testing.T) {
	got := GenerateMarkdownTemplate(ShapeReport, []Field{{Key: "done", Type: "boolean", Label: "Done"}})
...
func TestGenerate_ReportImageBlock(t *testing.T) {
	got := GenerateMarkdownTemplate(ShapeReport, []Field{{Key: "cover", Type: "image", Label: "Cover"}})
	if !strings.Contains(got, `![Cover]({{field "cover"}})`) {
		t.Errorf("image must render markdown image; got:\n%s", got)
	}
...
func TestGenerate_ReportListEachBlock(t *testing.T) {
	got := GenerateMarkdownTemplate(ShapeReport, []Field{{Key: "items", Type: "list"}})
...
func TestGenerate_ReportTableHasHeaderAndRowExpansion(t *testing.T) {
	got := GenerateMarkdownTemplate(ShapeReport, []Field{{Key: "rows", Type: "table"}})
...
func TestGenerate_ReportLoopWrapsInnerFields(t *testing.T) {
	...
	got := GenerateMarkdownTemplate(ShapeReport, fields)
...
func TestGenerate_MinimalNoFrontmatterNoDebug(t *testing.T) {
	got := GenerateMarkdownTemplate(ShapeMinimal, sampleFields())
	if strings.HasPrefix(got, "---") {
...
func TestGenerate_MinimalKeepsLoopAndImageBlocks(t *testing.T) {
	fields := []Field{
		{Key: "items", Type: "loopstart"},
		{Key: "cover", Type: "image", Label: "Cover"},
		{Key: "items", Type: "loopstop"},
	}
	got := GenerateMarkdownTemplate(ShapeMinimal, fields)
	if !strings.Contains(got, `{{#loop "items"}}`) {
		t.Errorf("minimal must render loop helpers")
	}
	if !strings.Contains(got, `![Cover]({{field "cover"}})`) {
		t.Errorf("minimal must render image markdown")
	}
}
...
func TestGenerate_TableHeaderAndRowPerField(t *testing.T) {
	got := GenerateMarkdownTemplate(ShapeTable, []Field{
		{Key: "title", Type: "text", Label: "Title"},
		{Key: "done", Type: "boolean", Label: "Done"},
		{Key: "tags", Type: "tags", Label: "Tags"},
	})
...
func TestGenerate_FrontmatterOnly(t *testing.T) {
	got := GenerateMarkdownTemplate(ShapeFrontmatter, []Field{
		{Key: "title", Type: "text", Label: "Title"},
